...
test.describe('User registration and login tasks.', () => {
  ...
  //
  test('(ATK-PW-1010) Log in via login form. @register-login @smoke', async ({ page, context }) => {
  ...
  //
  test('(ATK-PW-1011) Log in via POST. @register-login @smoke', async ({ page, context, request }) => {
    // TODO: Not ready yet.
  ...
  //
  test('(ATK-PW-1012) Log in via ULI. @register-login @smoke', async ({ page, context }) => {
    atkCommands.logInViaUli(page, context, 1)
  })
  ...
  test('(ATK-PW-1020) Create and delete user with Drush. @register-login @smoke', async ({ page }) => {
    atkCommands.deleteUserWithEmail(userEtherealAccount.userEmail, ['--delete-content'])
    atkCommands.createUserWithUserObject(userEtherealAccount, ['content_editor'])
    atkCommands.deleteUserWithUserName(userEtherealAccount.userName, [], ['--delete-content'])
  })

  //
  // Create a user with Drush from a fixture and delete it by UID.
  //
  test('(ATK-CY-1021) Create user with Drush, delete by UID.  @register-login @smoke', async ({ page }) => {
    atkCommands.deleteUserWithEmail(userEtherealAccount.userEmail, ['--delete-content'])
    atkCommands.createUserWithUserObject(userEtherealAccount, ['content_editor'])
    const uid = atkCommands.getUidWithEmail(userEtherealAccount.userEmail)
    atkCommands.deleteUserWithUid(uid, [], ['--delete-content'])
  })
})
